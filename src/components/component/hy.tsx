...
export default function Hy() {
    ...
    const handleCardClick = async (event: any, category: any, card: any) => {
        ...
        if (operation > 0) {
            // 检查数量限制
            if (userSelections.hySelection.filter((item: any) => item.id === card.id).length + 1 > 4 ) {
                alert('已选词条超出4个，请先移除后再重新选择！');
                return;
            }else {
                selectItem(category, card);
            }
        } else {
            if (userSelections.hySelection.reduce((acc: any, cur: any) => acc + (cur.id === card.id ? 1 : 0), 0) === 0) {
                return
            } else {
    ...
    const filteredHyCards = list.filter((card: any) =>
    ...
    return (
        <div>
            ...
            <div className="flex grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-6 p-4">
                {filteredHyCards.map((card: any) => {
                    const increase = calculateDamageIncrease('hySelection', card, userSelections);
                    const isSelected = userSelections.hySelection.some((selected: any) => selected.id === card.id);
                    ...
                    return (
                        <Card
                            ...
                            onClick={(event) => handleCardClick(event, 'hySelection', card)}
                        >
                            ...
                            <CardContent className="p-4 bg-background">
                                ...
                                <div className="mt-2 flex justify-between items-center">
                                    <span className="text-sm">
                                        选中数量: {userSelections.hySelection.reduce((acc, cur) => acc + (cur.id === card.id ? 1 : 0), 0)}
                                    </span>
                                </div>
                            </CardContent>
                        </Card>
